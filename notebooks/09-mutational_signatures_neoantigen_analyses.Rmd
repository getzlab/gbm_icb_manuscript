...
-   Extended Data Figure 2d, 2e

-   Extended Data Figure 2f

-   Figure 2f and Extended Data Figure 2g-i

-   Figure 2g and Extended Data Figure 2j-o

This script analyzes associations of TMB, mutational signatures, and neoantigen load with overall survival from ICB start.
...
# Figure 2f and Extended Data Fig. 2g-i
...
# Figure 2g and Extended Data Fig. 2j-o
...
tmb_cohort_full <- full_data %>%
  filter(!is.na(TMB_latest_pre) & !is.na(osicb)) %>%
  mutate(
    os_months = as.numeric(osicb) / 30.44,
    os_years = os_months / 12,
    status = if_else(Deceased == "Deceased", 1, 0),
    survival_group = factor(
      if_else(osicb >= 547, "LTS (>= 18 mo)", "STS (< 18 mo)"),
...
neo_cohort_full <- full_data %>%
  filter(!is.na(total_pre_neoantigen_burden) & !is.na(osicb)) %>%
  mutate(
    os_months = as.numeric(osicb) / 30.44,
    os_years = os_months / 12,
    status = if_else(Deceased == "Deceased", 1, 0),
