...
Add assignments to the augmented patient sheet
```{r}
sample_sheet <- read_tsv('../../../results/2024-06-20_cohort-sheets-180/sample_cohort_sheet.tsv', show_col_types = FALSE)
augmented_sheet <- read_tsv('../../../results/2025-06-06_single_cell_scores/augmented_pt_sheet.tsv')

assignments_merge <- sample_sheet %>% 
  inner_join(assignments, by = 'sample_id') %>% 
  select(sample_id_legacy, all_of(names(assignments)[-1]))

augmented_sheet <- augmented_sheet %>% 
  left_join(assignments_merge, by = c('sample_id_legacy_pre' = 'sample_id_legacy')) %>% 
  left_join(assignments_merge, by = c('sample_id_legacy_post' = 'sample_id_legacy'), suffix = c('_pre', '_post'))

write_tsv(augmented_sheet, '../../../results/2025-06-06_single_cell_scores/augmented_pt_sheet_with_single_cell_assignments_with_all_classifiers.tsv', na = '')
```

Correspondence with ssGSEA scores of Verhaak subtypes and MHC class I/II scores
```{r}
verhaak_scores <- read_excel('../../../results/2025-06-06_single_cell_scores/sTable8_Verhaak_ssGSEA_scores.xlsx', skip = 1)
mhc_scores <- read_tsv('../../../results/2024-08-09_mhc-ssgsea/mhc_ssgsea_results.tsv', show_col_types = FALSE)
