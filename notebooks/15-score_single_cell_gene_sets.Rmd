...
```{r}
tpm <- read_excel('../data/rnaseq_counts_tpms.xlsx', sheet = 2, skip = 1)
gbm_sigs_neftel <- read_excel('../data/Neftel_et_al_signatures.xlsx', skip = 4)
gbm_sigs_garofano <- read_excel('../data/Garofano_et_al_signatures.xlsx', sheet = 'Tab 26 - Supplementary Table 6j', skip = 2)
gbm_sigs_nomura <- read_excel('../data/Nomura_et_al_signatures.xlsx', sheet = 'TableS2', skip = 4)
gbm_sigs_couturier <- read_csv('../data/Couturier_et_al_signatures.csv')
...
write_tsv(comparison_cors, '../results/score_correlations.tsv')
...
ggsave(
  '../results/score_comparison_subset_paper.png',
...
ggsave(
  '../results/score_comparison_subset_paper.pdf',
  comparison_subset_plot,
  width = 12,
  height = 6,
  units = 'in'
...
```{r}
write_tsv(scores_3 %>% rownames_to_column('sample_id'), '../results/single_cell_scores_with_all_classifiers.tsv')
```
