...
* Extended Data Figure 4c, 4d, 4e, 4f, 4i, 4j
...
zenodo_dir_path <- "../data/bulk_transcriptional_classifiers"
...
#Object 5 & 6: HLA ssGSEA Survival (ExtDataFig 4j, Fig 4d)
data_hla_surv <- icb_data_raw %>%
  mutate(osicb_years = as.numeric(osicb) / 365.25)

# --- Object 7: MES vs PN Survival (Fig 4a) ---
data_fig4a <- icb_data_raw %>%
  mutate(osicb_years = as.numeric(osicb) / 365.25) %>%
  filter(
    verhaak_subtype_pre %in% c("MESENCHYMAL", "PRONEURAL"),
    !is.na(osicb_years),
...
```{r}
surv_object_subtype <- Surv(time = data_fig4a$osicb_years, event = data_fig4a$Deceased == "Deceased")
km_fit_subtype <- survfit(surv_object_subtype ~ subtype_factor, data = data_fig4a)

n_pn <- sum(data_fig4a$subtype_factor == "PRONEURAL")
n_mes <- sum(data_fig4a$subtype_factor == "MESENCHYMAL")
max_time_4a <- ceiling(max(data_fig4a$osicb_years, na.rm = TRUE)) 

plot_subtype <- ggsurvplot(km_fit_subtype, data = data_fig4a,
           pval = TRUE,
           pval.coord = c(max_time_4a * 0.9, 0.8),
           break.time.by = 1,
           xlim = c(0, max_time_4a),
           conf.int = FALSE,
           risk.table = FALSE,
...
# 1. All Cohort (<= 35 days) - Figure 4b
...
# 2. Newly Diagnosed (<= 35 days) - Figure 4c
...
mes_only_data <- analysis_data_4f %>% filter(mes == "Mesenchymal")
...
non_mes_data <- analysis_data_4f %>% filter(mes == "Non-Mesenchymal")
...
pn_only_data <- analysis_data_4f %>% filter(mvp == "Proneural")
...
Creates a Kaplan-Meier survival curve for patients stratified by high vs. low HLA class II ssGSEA scores.For the panel_Fig4a chunk:
